...
-- Create the function to handle new user creation with transaction handling
        ...
        WHILE counter < 10 LOOP
            ...
            EXCEPTION 
                ...
                WHEN OTHERS THEN
                    ...
                    RAISE WARNING 'Error in profile creation attempt %: % %', counter, SQLERRM, SQLSTATE;
                    -- Only abort if we've tried too many times
                    IF counter >= 9 THEN
                        RAISE EXCEPTION 'Failed to create profile after multiple attempts: % %', SQLERRM, SQLSTATE;
                    END IF;
            END;
        END LOOP;
        
        IF counter >= 10 THEN
            RAISE EXCEPTION 'Failed to find unique username after 10 attempts for user: %', NEW.id;
        END IF;
        
        RETURN NEW;
    EXCEPTION WHEN OTHERS THEN
        -- Log the final error
        RAISE WARNING 'Final error in handle_new_user: % %', SQLERRM, SQLSTATE;
...
-- Remove the problematic line and handle transactions within the function instead
